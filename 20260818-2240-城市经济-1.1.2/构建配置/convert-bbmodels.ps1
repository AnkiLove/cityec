...
function Convert-BbModel {
    ...
    foreach ($sourceElement in $model.elements) {
        ...
        foreach ($faceName in @('north', 'east', 'south', 'west', 'up', 'down')) {
            ...
            if ($TransparentBottom -and $faceName -eq 'down') {
                continue
            }
    ...
    if ($BottomFill) {
        $elements.Add([ordered]@{
            from = @($BottomFromX, 0.01, $BottomFromZ)
            to = @($BottomToX, 0.02, $BottomToZ)
            faces = [ordered]@{
                up = [ordered]@{
                    uv = @(0, 0, 16, 16)
                    texture = '#bottom'
    ...
    if ($FullBottom) {
        ...
        $elements.Add([ordered]@{
            from = @($BottomFromX, 0, $BottomFromZ)
            to = @($BottomToX, 1, $BottomToZ)
            faces = $bottomFaces
        })
    }
    ...
    if ($OpaqueBottom -or $FullBottom -or $BottomFill) {
...
Convert-BbModel -Source (Join-Path $downloads 'auto_teller_machine_off.bbmodel') -TargetName 'auto_teller_machine_bb_off' -TransparentBottom -BottomFill -BottomFromZ 0 -BottomToZ 10
Convert-BbModel -Source (Join-Path $downloads 'auto_teller_machine_on.bbmodel') -TargetName 'auto_teller_machine_bb_on' -TransparentBottom -BottomFill -BottomFromZ 0 -BottomToZ 10
Convert-BbModel -Source (Join-Path $downloads 'cash_register_off.bbmodel') -TargetName 'cash_register_bb_off' -YScale 1.0 -TransparentBottom -BottomFill -BottomFromX 2 -BottomToX 12 -BottomFromZ 3 -BottomToZ 13
Convert-BbModel -Source (Join-Path $downloads 'cash_register_on.bbmodel') -TargetName 'cash_register_bb_on' -YScale 1.0 -TransparentBottom -BottomFill -BottomFromX 2 -BottomToX 12 -BottomFromZ 3 -BottomToZ 13
